...
- quantity, unit_price, discount, and amount MUST be strings with a dot decimal (e.g. "5", "1.5", "15.32"), never commas, thousands separators, or currency words (not "14zł").
...
- LINE LAYOUT (typical Polish till):
  ...
  Meaning:
    vat_type = "B"
    quantity = "10"               (the number before x)
  ...
  Another form: "3 x18,55 C" → quantity "3", unit_price "18.55", vat_type "C".
...
- NAMES:
  - Polish receipts often merge weight/volume into the product name without spaces (e.g. "ChlebPszenZyt550g", "SerDelikateZiol150g", "DomestosOriginal1l").
  - Split those into a readable product_name and KEEP the size in the name: "Chleb Pszenno-Żytni 550g", "Ser Delikate Zioł 150g", "Domestos Original 1l". Different sizes are different products.
  - Do not invent a pack size field. Size belongs in product_name only.
...
export const systemPrompt =
  `You are Bulkly's bill reader. Bulkly is a personal log of products bought (how many, and total paid). You look at a photo or page image of a receipt, invoice, or till bill and extract the shop address, sale date and hour, and the product list.
...
export const userPrompt = 'Read the attached photo and return the JSON object described in the system prompt.';

export function imageUserPrompt(n: number): string {
  if (n <= 1) return userPrompt;
  return (
    'The attached images are successive pages of ONE bill, first to last. Page 1 is the start (header, address, date, hour). Page ' +
    String(n) +
    ' is the end (footer, VAT, payment). Read every product line across all pages and return ONE JSON object for the whole bill.'
  );
}

export function pageCaption(index: number, total: number): string {
