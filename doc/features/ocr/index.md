...
- each goods line: the till name, a cleaned name that **keeps printed size** (`550g`, `1.5l`), VAT type (A, B, or C), quantity (the number before `x`), the shelf unit price, any rabat, and the **final** line total when that figure is printed
...
If a line has a unit price and a rabat but no final paid figure, Bulkly fills the amount after reading: quantity × unit price − rabat. Printed totals are left as they are.
...
Quantity in the log is the till qty column. A weighed loose buy (`Marchew  1.450 x 4,99` per kg) is `1.450` kg. If a printed name includes a size (`Mąka 1kg`) and the qty column is 2, that is two items of that product. The same till line scanned several times (five milks at 3,29) stays as five separate rows.
