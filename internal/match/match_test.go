...
func TestProductMatchPriority(t *testing.T) {
	...
	chain := []Label{{ProductID: 7, Text: "Mąka"}, {ProductID: 7, Text: "Płatki"}}
	global := []Label{{ProductID: 4, Text: "Mąka"}, {ProductID: 4, Text: "Tortowa"}}
	names := []Label{{ProductID: 4, Text: "Cake flour"}, {ProductID: 5, Text: "Rice"}}

	id, ok := Product("MĄKA", shop, chain, global, names)
	if !ok || id != 5 {
		t.Fatalf("shop should win: id=%d ok=%v", id, ok)
	}

	id, ok = Product("Mąka", nil, chain, global, names)
	if !ok || id != 7 {
		t.Fatalf("chain should win over global: id=%d ok=%v", id, ok)
	}

	id, ok = Product("Tortowa", nil, chain, global, names)
	if !ok || id != 4 {
		t.Fatalf("global: id=%d ok=%v", id, ok)
	}

	id, ok = Product("Cake flour", nil, nil, nil, names)
	if !ok || id != 4 {
	...
	id, ok = Product("MAKA TORTOWA 1KG", nil, nil, []Label{{ProductID: 4, Text: "Mąka tortowa 1kg"}}, names)
	if !ok || id != 4 {
...
func TestProductRejectsFuzzy(t *testing.T) {
	...
	if _, ok := Product("Tortova", nil, nil, []Label{{ProductID: 4, Text: "Tortowa"}}, nil); ok {
		t.Fatal("alias typo should not match")
	}
	if _, ok := Product("Cak flour", nil, nil, nil, names); ok {
		t.Fatal("catalog typo should not match")
	}
	if _, ok := Product("MAKA TORTOWA 1KG", nil, nil, []Label{{ProductID: 4, Text: "Mąka tortowa"}}, names); ok {
		t.Fatal("shorter alias should not match")
	}
	if _, ok := Product("Rice 1kg", nil, nil, nil, names); ok {
		t.Fatal("trailing size should not match a catalog name")
	}
	if _, ok := Product("MAKA TORTOWA 1KG", nil, nil, nil, []Label{{ProductID: 4, Text: "Mąka"}}); ok {
...
func TestProductAmbiguous(t *testing.T) {
	...
	if _, ok := Product("Mąka", nil, nil, global, nil); ok {
	...
	if _, ok := Product("Rice", nil, nil, nil, names); ok {
		t.Fatal("two exact catalog hits should stay unmatched")
	}
}

func TestProductEmptyAndMiss(t *testing.T) {
	if _, ok := Product("", nil, nil, []Label{{ProductID: 1, Text: "Rice"}}, nil); ok {
		t.Fatal("empty query")
	}
	if _, ok := Product("Ghost", nil, nil, nil, []Label{{ProductID: 1, Text: "Rice"}}); ok {
		t.Fatal("miss")
	}
}
